...
//Construor da classe
Ligador::Ligador(std::vector<std::vector<std::string>> comandos_entrada, bool is_main){
...
bool Ligador::check_int(std::string s){
    bool check = 1;
    unsigned int i = 0;
...
void Ligador::const_to_value(){
    ...
    for (i = 0; i < this->comandos_entrada.size(); i++){
        for  (j = 0 ; j < this->comandos_entrada[i].size(); j++ ){
            ...
            if( this->mapa.find(this->comandos_entrada[i][j]) != mapa.end() ){    
...
//Calcula os 4 inteiros principais para o Ligador
void Ligador::calcula_valores_pilha(){
...
//triste :(
int Ligador::check_instrucao(std::string s){
    int operacao = -1;
